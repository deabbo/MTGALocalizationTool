...
for card_file in card_files:
    ...
    try:
        ...
        card_values_to_update = [
            ...
            # v.1.2.2 866018,866017 추가
            ('866018', '연글라이더 절도범이 들어올 때, 각 플레이어에 대해, 그 플레이어가 조종하는 다른 마법물체 또는 생물을 최대 한 개까지 목표로 정한다. 연글라이더 절도범이 전장에 남아있는 한, 선택된 지속물들은 "{oT}, 이 마법물체를 희생한다: 원하는 색의 마나 한 개를 추가한다."를 가진 보물 마법물체가 되며 다른 모든 능력을 잃는다.', ''),
            ('866017', 'CARDNAME이 들어올 때, 각 플레이어에 대해, 그 플레이어가 조종하는 다른 마법물체 또는 생물을 최대 한 개까지 목표로 정한다. CARDNAME이 전장에 남아있는 한, 선택된 지속물들은 "{oT}, 이 마법물체를 희생한다: 원하는 색의 마나 한 개를 추가한다."를 가진 보물 마법물체가 되며 다른 모든 능력을 잃는다.', ''),
            ...
            ('548199', '당신의 손에 대지 카드가 없다면, 대지 카드 한 장 및 대지가 아닌 카드 한 장을 탐색한다. 그렇지 않다면, 대지가 아닌 카드 두 장을 탐색한다.', ''),
            ...
            ('11', '대지',''),
            # v.1.2.3
            ('3666', '상대를 목표로 정한다. 그 상대는 생명 3점을 잃고 당신은 생명 3점을 얻는다.',''),
            ('731439', '종막',''),
            ('333223', '주문숙련',''),
            ('498187', '#NoTranslationNeeded',''),
            ('570446', '뱀파이어 기사 토큰',''),
            ('570586', '엘드라지 괴수 토큰',''),
            ('595534', '',''),
            ('595534', '#NoTranslationNeeded',''),
            ('706713', '당신이 무덤에서 발동하는 주문들은 발동하는 데 {oU}이(가) 덜 든다.',''),
            ('739602', '부여된 생물은 "이 생물이 당신의 언탭단에 언탭되려고 하면, 대신 이 생물에서 +1/+1 카운터 한 개를 제거한다. 그렇게 한다면, 이 생물을 언탭한다."를 가진다.',''),
            ('743343', '{oT}: 당신의 무덤에 있는 마법물체 카드를 목표로 정한다. 당신은 이 턴에 그 카드를 발동할 수 있다.',''),
            ('750872', '상대가 조종하며 마나 값이 X 이하인 대지가 아닌 지속물을 목표로 정한다. 그 지속물을 소유자의 서고에 섞어넣는다. X는 이런 식으로 탭된 생물의 수다.',''),
            ('756020', '그 플레이어가 이런 식으로 생물을 희생할때, 당신은 카드 한 장을 버릴 수 있다. 그렇게 한다면, 희생된 생물의 복제본을 당신의 손으로 부른다. 그 카드는 영구적으로 "원하는 유형의 마나를 지불해 이 주문을 발동할 수 있다."를 얻는다.',''),
            ('764159', '하나를 선택한다. 무덤에 카드가 여덟 장 이상 있는 상대가 있다면, 대신 당신은 둘 모두를 선택할 수 있다. • 주문을 목표로 정한다. 그 주문을 복사한다. 당신은 그 복사본의 목표를 새로 정할 수 있다. • 생물을 목표로 정한다. 그 생물의 복사본인 토큰 한 개를 만든다.',''),
            ('795857', '이 턴에 다음으로 당신이 순간마법 또는 집중마법 주문을 발동할 때, 그 주문을 복사한다. 당신은 그 복사본의 목표를 새로 정할 수 있다.',''),
            ('797712', '당신이 이 턴에 다음 생물 주문을 발동할 때, 당신의 서고에서 그보다 마나 값이 낮은 생물 카드 한 장을 찾아, 그 카드를 전장에 놓은 후, 서고를 섞는다.',''),
            ('814534', '카드 유형으로부터의 보호',''),
            ('832297', '당신이 이번 턴에 공룡 주문을 발동할 때마다, 그 주문은 "이 생물이 들어올 때, 다른 생물을 목표로 정한다. 당신은 이 생물과 그 생물이 싸우게 할 수 있다."를 얻는다.',''),
            ('837771', '당신이 조종하는 모험을 가진 각 생물은 +1/+1 카운터 한 개를 추가로 가지고 들어온다.','당신이 조종하는 모험을 가진 각 생물은 <nobr>+1/+1</nobr> 카운터 한 개를 추가로 가지고 들어온다.'),
            ('838395', '신속 및 "이 생물이 들어올 때, 원하는 목표를 정한다. 이 생물은 그 목표에게 피해 1점을 입힌다."를 가진 1/2 적색 및 백색 인간 도적 생물 토큰 한 개를 만든다.',''),
            ('838397', '당신의 유지단 시작에, 무작위로 한 개를 선택한다. 해당 특성을 지닌 적색 및 백색 생물 토큰 한 개를 만든다. — • 돌진 및 신속을 가진 3/1 인간 전사. • 생명연결 및 신속을 가진 2/1 인간 성직자. • 신속 및 "이 생물이 들어올 때, 원하는 목표를 정한다. 이 생물은 그 목표에게 피해 1점을 입힌다."를 가진 1/2 인간 도적.','당신의 유지단 시작에, 무작위로 한 개를 선택한다. 해당 특성을 지닌 적색 및 백색 생물 토큰 한 개를 만든다.<nobr> —</nobr> • 돌진 및 신속을 가진 3/1 인간 전사. • 생명연결 및 신속을 가진 2/1 인간 성직자. • 신속 및 "이 생물이 들어올 때, 원하는 목표를 정한다. 이 생물은 그 목표에게 피해 1점을 입힌다."를 가진 1/2 인간 도적.'),
            ('839031', '성좌 — 부여마법이 당신의 조종하에 전장에 들어올 때마다, 다른 생물을 목표로 정한다. 당신은 이 지속물이 이 능력을 가진 채로 그 생물의 복사본이 되게 할 수 있다.','<i>성좌</i><nobr> —</nobr> 당신이 조종하는 부여마법이 들어올 때마다, 다른 생물을 목표로 정한다. 당신은 이 지속물이 이 능력을 가진 채로 그 생물의 복사본이 되게 할 수 있다.'),
            ('853085', '이 생물이 들어올 때, 당신이 조종하지 않는 생물을 최대 한 개까지 목표로 정한다. 이 지속물은 그 생물과 싸운다.',''),
            ('853086', '당신이 늑대 또는 늑대인간 생물 주문을 발동할 때, 그 주문은 \'이 생물이 들어올 때, 당신이 조종하지 않는 생물을 최대 한 개까지 목표로 정한다. 이 생물은 그 생물과 싸운다.\'를 얻는다.',''),
            ('853087', '당신은 "당신이 늑대 또는 늑대인간 생물 주문을 발동할 때, 그 주문은 \'이 생물이 들어올 때, 당신이 조종하지 않는 생물을 최대 한 개까지 목표로 정한다. 이 생물은 그 생물과 싸운다.\'를 얻는다."를 가진 일회성 축복을 받는다.',''),
            ('870006', '부여된 플레이어의 두 번째 본단계 시작에, 이 단계 이후에 추가적인 시작단계가 생긴다.',''),
            ('877455', '당신이 조종하는 생물을 탭 또는 언탭하세요.({o4}, {oT}: 카드 한 장을 뽑는다. 이 능력이 가진 활성화비용의 각 마나에 대해, 당신은 그 마나를 지불하는 대신 당신이 조종하는 언탭된 생물을 탭할 수 있다. 집중마법 시기에만 활성화할 수 있다.)',''),
            ('881012', '당신이 주문을 발동할 때마다, 그 주문을 복사한다. 당신은 그 복사본의 목표를 새로 정할 수 있다.',''),
        ]
